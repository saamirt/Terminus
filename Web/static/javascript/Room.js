...
function Room(roomname, introtext, roompic, map_loc) {
	this.parents = [];
	this.children = new Array();
	this.items = new Array();
	this.commands = ["cd", "ls", "less", "man", "help", "exit", "pwd"];
	this.room_name =
		typeof roomname === "undefined" ? "Generic Room" : roomname;
	this.room_pic =
		typeof roompic === "undefined"
			? "./static/img/none.gif"
			: "./static/img/" + roompic;
	this.map_loc = map_loc && map_loc.length >= 2 ? map_loc : ["0px", "0px"];
	console.log(map_loc, this.map_loc);
	this.intro_text =
		typeof introtext === "undefined" ? "This is a simple room" : introtext;
...
Room.prototype.display = function(room = this) {
	$("#scene").attr("src", room.room_pic); // Display image of room
	$("#map__img").css({
		"-webkit-transform": `translate(${room.map_loc[0]},${room.map_loc[1]})`,
		"-moz-transform": `translate(${room.map_loc[0]},${room.map_loc[1]})`,
		"-ms-transform": `translate(${room.map_loc[0]},${room.map_loc[1]})`,
		"-o-transform": `translate(${room.map_loc[0]},${room.map_loc[1]})`,
		transform: `translate(${room.map_loc[0]},${room.map_loc[1]})`
	}); // Display map
	return "";
};
	...
	if (args.length > 0) {
		if (this.childrenStringArray().indexOf(args[0]) > -1) {
			return this.children[
				this.childrenStringArray().indexOf(args[0])
			].printLS();
		} else {
	...
	if (args.length < 1) {
		...
		for (var i = 0; i < this.items.length; i++) {
			if (item === this.items[i].toString()) {
				...
				if (
					this.room_name === "Library" &&
					item === "InconspicuousLever"
				) {
...
Room.prototype.mv = function(args) {
	if (args.length != 2) {
		...
		if (
			item_name_to_move >= 0 &&
			this.childrenStringArray().indexOf(args[1]) >= 0
		) {
			itemtoadd = this.items[this.itemStringArray().indexOf(args[0])];
			this.children[this.childrenStringArray().indexOf(args[1])].addItem(
				itemtoadd
			);
			if (
				args[0] === "UglyTroll" &&
				this.room_name === "CaveOfDisgruntledTrolls"
			) {
	...
	if (args.length < 1) {
		...
		for (var i = 0; i < args.length; i++) {
			if (this.getItemFromName(args[i]) != -1) {
				if (
					this.getItemFromName(args[i]).valid_cmds.indexOf("rm") > 0
				) {
					var removedItem = this.removeItem(args[i]);
					if (
						removedItem.itemname === "ThornyBrambles" &&
						this.room_name === "OminousLookingPath"
					) {
						this.ev.fire("rmBrambles");
					}
					if (
						removedItem.itemname === "UglyTroll" &&
						this.room_name === "CaveOfDisgruntledTrolls"
					) {
						this.ev.fire("openSlide");
					}
					if (
						removedItem.itemname === "LargeBoulder" &&
						this.room_name === "RockyPath"
					) {
...
Room.prototype.grep = function(args) {
	if (this.commands.indexOf("grep") > 0) {
		...
		if (this.getItemFromName(args[1]) != -1) {
			var item_to_find_in_text = this.getItemFromName(args[1]).cmd_text[
				"less"
			];
			var line_array = item_to_find_in_text.split("\n");
			var return_arr = jQuery.grep(line_array, function(line) {
		...
		if (item_to_copy != -1) {
			...
			if (this.room_name === "ArtisanShop") {
				if (
					...
					if (
						this.getItemFromName("gear1") != -1 &&
						this.getItemFromName("gear2") != -1 &&
						this.getItemFromName("gear3") != -1 &&
						this.getItemFromName("gear4") != -1 &&
						this.getItemFromName("gear5") != -1
					) {
				...
				if (
					item_to_copy_name === "EarOfCorn" &&
					new_item_name === "AnotherEarOfCorn"
				) {
...
Room.prototype.mkdir = function(args) {
	if (this.commands.indexOf("mkdir") > 0) {
		if (args.length === 1) {
			...
			if (
				this.room_name === "Clearing" &&
				room_name_to_make === "House"
			) {
