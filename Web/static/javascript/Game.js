...
var man_pages = {
	...
	less:
		...
		"Rememberrrrrr...",
	man:
		"I'm the old man dangit! You can't try to get more information about me. Here are all the commands you can man: cd, ls, rm, mv, exit, help, man, touch, grep, pwd.",
	...
	$("#term").terminal(
		function(input, term) {
			...
			if (current_room.commands.indexOf(command) > -1) {
				//Could use current_room.hasOwnProperty(command)
				...
				if (args.length > 0 && args[0].indexOf("/") > 0) {
					...
					for (var i = 0; i < rooms_in_order.length; i++) {
						...
						if (i > 0 && rooms_in_order[i - 1] === "~") {
							...
							cur_room_to_test = cur_room_to_test.can_cd(
								room_to_cd
							);
						}
						if (
							(command === "cd" || command === "ls") &&
							cur_room_to_test === false
						) {
					...
					if (room_num == path_rooms.length - 1) {
							...
							for (
								...
								if (
									...
									substring_matches.push(
										search_room.children[child_num]
											.room_name
									);
								}
							}
						...
						if (
							...
							for (
								...
								if (
									search_room.items[item_num].itemname.match(
										"^" + path_rooms[room_num]
									)
								) {
									substring_matches.push(
										search_room.items[item_num].itemname
									);
								}
							}
						}
						...
						if (substring_matches.length == 1) {
							...
							$("#term")
								.terminal()
								.set_command(split_command.join(" "));
						}
						//If multiple matches exist
							...
							do {
								...
								i == substring_matches.length &&
								++lCSindex
							);

							var longestCommonSubstring = substring_matches[0].slice(
								0,
								lCSindex
							);
							...
							if (longestCommonSubstring != "") {
								...
								if (
									...
									$("#term")
										.terminal()
										.echo(substring_matches.join(" ")); //Print the matches
									$("#term")
										.terminal()
										.set_command(
											split_command
												.join(" ")
												.replace(/\s+$/, "")
										); //Set the text to what the user entered
								}
								//Otherwise, fill in the longest common substring
									...
									$("#term")
										.terminal()
										.set_command(split_command.join(" ")); //Set the terminal text to this auto-completion
								}
							}
